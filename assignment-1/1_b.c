...
void tokenize(char *raw_input, char **args, int *num_args);
...
void execute_command(char **args, bool block);
bool blocking(char **args, int *num_args);
...
int main(void)
{
    ...
    while (should_run)
    {
        ...
        */
        ...
        block = blocking(args, &num_args);

        // Non-blocking wait to collect zombies using polling
        // alternative would be to use signal with SIGCHLD
        int status;
        while (waitpid(-1, &status, WNOHANG) > 0)
            ;

        execute_command(args, block);
    }
...
void append(struct node **root, char *raw_input)
{
    ...
    */
    ...
    struct node *node = *root; // start at root
...
void execute_command(char **args, bool block)
{
    ...
    */
    ...
    if (pid == 0) // child
    {
        ...
        execvp(args[0], args);
        perror(args[0]); // execvp only returns if error occurred
        exit(1);         // must exit to kill child
    }
    else // parent
    {
        // handle case of &
...
{
    /*
      determine if command should be blocking
     */
    bool block = true;
